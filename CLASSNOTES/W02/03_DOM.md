...
1. Reference: Manipulating documents – MDN.
1. Note that the Active learning: A dynamic shopping list example found at the end of the MDN article will help you complete this week's activities.

1. Practice: Common DOM manipulation concepts and use cases.
1. Select an HTML element from the document using the querySelector method.
1. This line of code selects the first instance of an article element from the document and assigns it as a reference to the variable named article.

```js
const article = document.querySelector('article');
```

1. Change the innerHTML property of an existing element.
1. This line of code uses an existing variable that references a previously selected element and changes its innerHTML property value.

```js
article.innerHTML = 'innerHTML supports <strong>HTML</strong> tags. The textContent property does not.';
```

1. Change the inline CSS style of an element.
1. This line of code changes the text-align CSS property of the selected element.

```js
article.style.textAlign = 'right';
```

1. Change an attribute of an element.
1. This line of code adds a class attribute to the element and assigns it a value.
...
1. This method is often preferred for class manipulation because classList provides additional methods like add, remove, toggle, and contains, making it more convenient and expressive for managing classes.
1. Create an element.
1. This line of code creates a new <p> element and stores it in the variable named paragraph.

```js
const paragraph = document.createElement('p');
```

1. Append content or elements to an element.
1. These lines of code add content to the end of the article element.
...
1. Remove an element from the document.
1. These lines of code remove the paragraph from the article and then remove the article itself.
...
1. Your bom.html HTML document should include the basic meta tags and an appropriate title.
1. Create an external CSS file and a JavaScript file, placing them in appropriate subfolders within the week02 folder.
...
1. Copy and paste the basic interface (the HTML and CSS) from the following CodePen ☼ BOM Top 10.
1. In your blank JavaScript file, declare three variables that hold references to the input, button, and list elements.
...
1. We named this variable li for simplicity.
1. The variable name does not have to match the element type being created.
...
1. textContent is preferred over innerHTML because it is more secure.
1. However, if you need to include HTML tags, use innerHTML.
1. textContent will not render HTML tags; it will display them as plain text.
...
<https://codepen.io/BYU-Idaho/pen/QwwKMzL>
