...
1. Practice: Common DOM event concepts and use cases. These are just a few examples; there are many other events and use cases.
1. Click Event: Triggered when a user clicks on an element. It's widely used for handling button clicks, link clicks, or any interaction involving a mouse click.
...
1. Open your JavaScript file that is supporting the bom.html application, if needed.
1. Create a click event listener for the Add Chapter button with addEventListener.
...
1. Within the Add Chapter button click event function block (between the opening and closing braces of the callback function { ... }), complete the following tasks:
1. Check to make sure the input is not blank before completing the remaining tasks in this list. Use an if block that either provides a message or does nothing and returns the .focus() to the input field.
...
1. When the user clicks the Add Chapter button, the program checks to make sure that the input field is not empty. If the input is valid, the program creates and populates the list elements.
1. Add an event listener to the delete button that removes the li element when clicked.
...
<https://codepen.io/BYU-Idaho/pen/bNerGjQ>
